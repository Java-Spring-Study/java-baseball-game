...
public class GameController {
  ...
  private int countBall(List<Integer> playersNum) {
    int count = 0;
    for (int i = 0; i < Constant.LIST_SIZE; i++) {
        if (computer.getRandomNumberList().contains(playersNum.get(i)) && !(playersNum.get(i)
            .equals(computer.getRandomNumberList().get(i)))) {
            count += 1;
        }
    }
    return count;
  ...
  public void printScore() {
    if (ball > 0 && strike > 0) {
      System.out.println(ball + GameControllerMessage.BALL_STRING.getMessage() + strike
          + GameControllerMessage.STRIKE_STRING.getMessage());
    } else if (ball > 0) {
      System.out.println(ball + GameControllerMessage.BALL_STRING.getMessage());
    } else if (strike > 0) {
      System.out.println(strike + GameControllerMessage.STRIKE_STRING.getMessage());
    } else {
      System.out.println(GameControllerMessage.NOTHING.getMessage());
    }
  }
  ...
  void gameClearMessage() {
    System.out.println(GameControllerMessage.ALL_CORRECT.getMessage());
  }

  private void judgeGameRestart(int playersAnswer) {
    if (playersAnswer == RestartStatus.RESTART.getRestartStatus()) {
      setRestartStateTrue();
    } else {
